...
function drawSineWave() {
    ...
    for (let x = 0; x < width; x++) {
        const y = centerY - amplitude * Math.cos((2 * Math.PI * x) / waveLength);
        ctx.lineTo(x, y);
    }
...
class Ball {
    ...
    update() {
        if (!this.onWave) {
            ...
            const waveY = centerY - amplitude * Math.cos((2 * Math.PI * this.x) / waveLength);
            ...
            const waveSlope = Math.sin((2 * Math.PI * this.x) / waveLength); // Eğri eğimi
            this.vx += waveSlope * 0.7; // Eğime göre hızlanma
            this.vx *= friction; // Sürtünme etkisi
            this.x += this.vx;

            // Sinüs dalgasındaki y konumunu güncelle
            this.y = centerY - amplitude * Math.cos((2 * Math.PI * this.x) / waveLength);
